...
import { EcgTrace } from "@/components/work/ecg-trace";
import {
  royCardiologyMeta,
  royCardiologySections,
  type RoySection,
} from "@/lib/roy-cardiology-case-study";
import { cn } from "@/lib/utils";

function SectionPhoto({ image }: { image: RoySection["image"] }) {
  return (
...
function NarrativeSection({ section }: { section: RoySection }) {
...
export function RoyCardiologyCaseStudy() {
  return (
    <>
      {/* Compact hero — white body starts above the fold on laptop */}
      <section className="relative overflow-hidden bg-chocolate text-stone">
        <div className="absolute inset-0">
          <Image
            src={royCardiologyMeta.heroImage}
            alt=""
            fill
            priority
            sizes="100vw"
