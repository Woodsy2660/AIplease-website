...
import { DataFlowTrace } from "@/components/work/data-flow-trace";
import {
  pacificFertiliserMeta,
  pacificFertiliserSections,
  type PacificFertiliserSection,
} from "@/lib/pacific-fertiliser-case-study";

function SectionPhoto({ image }: { image: PacificFertiliserSection["image"] }) {
  return (
...
function NarrativeSection({ section }: { section: PacificFertiliserSection }) {
...
export function PacificFertiliserCaseStudy() {
  return (
    <>
      <section className="relative overflow-hidden bg-chocolate text-stone">
        <div className="absolute inset-0">
          <Image
            src={pacificFertiliserMeta.heroImage}
            alt=""
            fill
            priority
            sizes="100vw"
        ...
        <div className="relative mx-auto max-w-6xl px-5 pb-0 pt-6 sm:px-6 sm:pt-7 lg:px-8 lg:pt-8">
          ...
          <div className="mt-5 flex items-center gap-4">
            <Image
              src={pacificFertiliserMeta.logo}
              alt="Pacific Fertiliser logo"
              width={44}
              height={44}
              className="size-11 shrink-0 rounded-full border border-stone/20 bg-white object-contain p-1"
            />
            <p className="font-mono text-[0.6875rem] font-medium uppercase tracking-[0.16em] text-sky">
              {pacificFertiliserMeta.eyebrow}
            </p>
          </div>

          <h1 className="mt-4 max-w-3xl font-sans text-[clamp(1.625rem,3.2vw,2.5rem)] font-bold leading-[1.1] tracking-[-0.03em] text-stone">
            {pacificFertiliserMeta.headline}
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-stone/75 sm:text-base">
            {pacificFertiliserMeta.standfirst}
          </p>
        </div>
      ...
      <article className="bg-white">
        {pacificFertiliserSections.map((section) => (
